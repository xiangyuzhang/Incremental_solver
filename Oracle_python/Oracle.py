...
class Oracle:
    ...
    def __assign(self):
        for key in self.PI_temp.keys():
            self.assign.append(str(key) +  " " + self.PI_temp[key] + "\n")
    ...
    def __grab(self):
        ...
        if sat == True:
            ...
            for node in self.posIndex:
                if "-" in nodeVals[node - 1]: self.PO_temp[node] = "0"
                else: self.PO_temp[node] = "1"
...
def runmain():
    parser = argparse.ArgumentParser(usage='python Oracle.py <orac.v> <PI.txt> [-o <PO>]]', description='This program will be used to queryOrac',)
